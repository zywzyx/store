...
def add_user():
    ...
    bank = "中国工商银行"
    sql = "select count(*) from IABC"
    data = select(sql, [])
    if data[0][0] >= 100:
        print("账户已满")
        return 3
    sql1 = "select * from IABC where code = %s"
    ...
    if len(data1) != 0:
        ...
        sql2 = "insert into iabc values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        param2 = [code, name, pwd, country, province, street, house, money, bank]
        update(sql2, param2)
        sql3 = "select * from iabc where code = %s"
...
def save_money():
    code = input('请输入账号:')
    sql = "select * from iabc where code = %s"
    data = select(sql, code)
    if len(data) == 1:
        money = float(input("请输入存款金额:"))
        money = money + float(data[0][7])
        print(money)
        sql1 = "update iabc set money=%s where code=%s"
        parma = [money, code]
        update(sql1, parma)
...
def tr_money():
    ...
    if len(data) == 1:
        ...
        if pwd == data[0][2]:
            ...
            sql4 = "select * from abc where code = %s"
            ...
            if len(data1) == 1:
                ...
                print(money)
                #转出账户转出金钱
                sql2 = "update iabc set money=%s where code= %s"
                parma2 = [money, code]
                update(sql2, parma2)
                ...
                print(info % (
                    ...
                    data5[0][8]))
                #转入账户收到金钱
                sql3 = "update iabc set money=%s where code= %s"
                ...
                print(money)
                # 转出账户转出金钱
                sql5 = "update iabc set money=%s where code= %s"
                parma5 = [money, code]
                update(sql5, parma5)
                data5 = select(sql, code)
                print("转账成功！")
                print(info % (
                    ...
                    data5[0][8]))
                # 转入账户收到金钱
                sql6 = "update abc set money=%s where code= %s"
